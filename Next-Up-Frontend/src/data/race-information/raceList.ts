...
const skillNames: string[] = Object.keys(skillList).map((key: string) => {
	return key;
});

export const raceList: { [key: string]: RaceListTypes } = {
	Android: {
		raceSource: 'Core',
		raceName: 'Android',
	...
	Lashunta: {
		...
		raceName: 'Lashunta',
		raceScoreModifiers:
			'+2 Cha, +2 Str, -2 Wis (Korasha) or +2 Cha, +2 Int, -2 Con (Damaya)',
		raceDescription:
		...
		optionDescription: [
			...
			'Select your second skill. It must me different from the first selected.',
		],
		optionArray: [['Korasha', 'Damaya'], skillNames, skillNames],
